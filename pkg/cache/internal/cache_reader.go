...
func (c *CacheReader) List(ctx context.Context, out client.ObjectList, opts ...client.ListOption) error {
	...
	case listOpts.FieldSelector != nil:
		...
		// namespace.
		objs, err = byIndexes(c.indexer, listOpts.FieldSelector.Requirements(), listOpts.Namespace, clusterName)
...
func byIndexes(indexer cache.Indexer, requires fields.Requirements, namespace string, cluster logicalcluster.Name) ([]interface{}, error) {
	var (
		err  error
		objs []interface{}
		vals []string
	...
	for idx, req := range requires {
		...
		if cluster.Empty() {
			...
			indexedValue = KeyToClusteredKey(cluster.String(), namespace, req.Value)
		}
		if idx == 0 {
